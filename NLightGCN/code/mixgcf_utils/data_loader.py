...
def statistics(train_data, valid_data, test_data):
    ...
    if dataset not in ['yelp2018','gowalla']:
        n_items -= n_users
...
def build_sparse_graph(data_cf):
    ...
    vals1 = [1.] * len(cf[:, 0])
    vals2 = [1.] * len(cf[:, 0])
    mat = _bi_norm_lap(sp.coo_matrix((vals, (cf_[:, 0], cf_[:, 1])), shape=(n_users+n_items, n_users+n_items)))
    mat1 = _si_norm_lap(sp.coo_matrix((vals1, (cf[:, 0], cf[:, 1])), shape=(n_users, n_items)))
    mat2 = _si_norm_lap(sp.coo_matrix((vals2, (cf[:, 1], cf[:, 0])), shape=(n_items, n_users)))
    pdb.set_trace()
    mat3 = mat2.dot(mat1)
    return mat.tocoo(), mat1.tocoo(), mat2.tocoo(), mat3.tocoo()
...
def load_data(model_args):
    ...
    norm_mat, norm_ui, norm_iu, norm_u2 = build_sparse_graph(train_cf)
    ...
    print('loading over ...')
    return train_cf, user_dict, n_params, norm_mat, norm_ui, norm_iu, norm_u2
